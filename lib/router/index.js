...
Router.prototype.flush = function(routes) {
  ...
  _.each(sortedRouteAddresses, function(address) {
    ...
    // Warn about paths that end with pattern variables or wildcards that don't explicitly declare `skipAssets: true` or `skipAssets: false`
    if ((address.match(/\:[^\/]+$/) || address.match(/\/\*$/)) && _.isUndefined(target.skipAssets)) {
      sails.log.debug('Warning: route `' + address + '` should explicitly declare `skipAssets: true` or `skipAssets: false` to ensure correct handling of assets!');
      sails.log.debug('See http://sailsjs.com/docs/concepts/routes/url-slugs for more info.');
      sails.log.debug();
    }
 ...
 */
Router.prototype.getActionIdentityForTarget = function(target) {
  ...
  if (_.isObject(target) && !_.isArray(target) && !_.isFunction(target)) {
    ...
    if (target.controller) {
      if (!target.action) {
        throw flaverr('E_NOT_ACTION_TARGET', new Error('If `controller` is specified, `action` must be also!'));
      }
  ...
  // 'user/create'
    ...
    throw flaverr('E_NOT_ACTION_TARGET', new Error('Target must be a dictionary or string!'));
  }
