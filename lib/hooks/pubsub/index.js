  ...
  return {
    ...
    initialize: function(cb) {
      ...
      if (!sails.hooks.sockets || !sails.hooks.orm) {
        sails.log.verbose('Cannot use `pubsub` hook without the `sockets` and `orm` hooks enabled!\n');
      ...
      publish: function(ids, data, req) {
        ...
        if (!_.isArray(ids) || _.any(ids, function(id) {return !_.isString(id) && !_.isNumber(id);})) {
          ...
          try {
            ...
            throw new Error('We tried to transform `' + util.inspect(ids, {depth: 2}) + '` into an array of IDs, but some values were `null` or `undefined`!');
          }
      ...
      _publishDestroy: function (id, req, options) {
        ...
        if (options.previous) {
          ...
          _.each(this.associations, function(association) {
            ...
            else if (association.type === 'collection' && association.via && previous[association.alias].length) {
