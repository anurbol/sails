...
module.exports = function(sails) {
  ...
  return {
    ...
    configure: function() {
      ...
      if (sails.config.http.trustProxy === 0 || sails.config.http.trustProxy === '' || sails.config.http.trustProxy === null || isNaN(sails.config.http.trustProxy)) {
        throw flaverr('E_HTTP_BAD_TRUSTPROXY', new Error(
          '\n-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-\n'+
