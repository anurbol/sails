  ...
  return {
    ...
    configure: function() {
      ...
      // Bail out if `allowOrigins` is not an array or `*`.
        ...
        throw new Error('Invalid global CORS settings: if `allowOrigins` is specified, it must be either \'*\' or an array of strings.  See http://sailsjs.com/config/security for more info.');
      }
      ...
      // If `allowOrigins` is not `*` and not an array at this point, bail.
        ...
        throw flaverr('E_BAD_ORIGIN_CONFIG', new Error('Invalid global CORS settings: if `sails.config.security.cors.allowOrigins` is specified, it must be \'*\' or an array of strings.'));
      }
      ...
      if (_.isArray(sails.config.security.cors.allowOrigins)) {
        try {
          ...
          if (e.code === 'E_INVALID') {
            throw new Error('Invalid global CORS `allowOrigins` setting: ' + e.message+'  (See http://sailsjs.com/config/security for help.)');
          }
