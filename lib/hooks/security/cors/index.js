    ...
    sails.on('router:before', function () {
      ...
      _.each(sails.router.explicitRoutes, function(routeConfig, route) {
        ...
        if (routeCorsConfig !== false && routeConfig.action === 'security/grant-csrf-token') {
          sails.log.debug('Warning: using the `grant-csrf-token` for cross-origin requests is not supported in all browsers.');
          sails.log.debug('(You are seeing this message because the route `' + verb + ' ' + path + '` has CORS settings configured.)');
          sails.log.debug();
        }
