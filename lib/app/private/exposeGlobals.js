...
module.exports = function exposeGlobals() {
  ...
  // If globals config is provided, it must be either `false` or a dictionary.
    ...
    throw flaverr('E_BAD_GLOBAL_CONFIG', new Error('As of Sails v1, if `sails.config.globals` is defined, it must either be `false` or a dictionary (plain JavaScript object) or `false`.  But instead, got: '+util.inspect(sails.config.globals, {depth:null})+'\n> Note: if no globals config is specified, Sails will now assume `false` (no globals).  This is to allow for more intuitive programmatic usage.\nFor more info, see http://sailsjs.com/config/globals'));
  }
  ...
  if (sails.config.globals._ !== false) {
    if (!_.isObject(sails.config.globals._)) {
      throw flaverr('E_BAD_GLOBAL_CONFIG', new Error('As of Sails v1, `sails.config.globals._` must be either `false` or a locally-installed version of Lodash (typically `require(\'lodash\')`).  For more info, see http://sailsjs.com/config/globals'));
    }
  ...
  if (sails.config.globals.async !== false) {
    if (!_.isObject(sails.config.globals.async)) {
      throw flaverr('E_BAD_GLOBAL_CONFIG', new Error('As of Sails v1, `sails.config.globals.async` must be either `false` or a locally-installed version of `async` (typically `require(\'async\')`)  For more info, see http://sailsjs.com/config/globals'));
    }
  ...
  if (sails.config.globals.sails !== false) {
    if (sails.config.globals.sails !== true) {
      throw flaverr('E_BAD_GLOBAL_CONFIG', new Error('As of Sails v1, `sails.config.globals.sails` must be either `true` or `false` (Tip: you may need to uncomment the `sails` setting in your `config/globals.js` file).  For more info, see http://sailsjs.com/config/globals'));
    }
    global['sails'] = sails;
  }
  ...
  if (sails.config.globals.models !== false && sails.config.globals.models !== true) {
    throw flaverr('E_BAD_GLOBAL_CONFIG', new Error('As of Sails v1, `sails.config.globals.models` must be either `true` or `false` (you may need to uncomment the `models` setting in your `config/globals.js` file).  For more info, see http://sailsjs.com/config/globals'));
  }
