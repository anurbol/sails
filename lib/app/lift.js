...
module.exports = function lift(configOverride, done) {
  ...
  // Callback is optional (but recommended.)
    ...
    if (err) {
      ...
      sails.log.error('(You are seeing this error message because no callback was provided to `.lift()`.)');
      return;
    }

    sails.log.verbose('App lifted successfully.');
    sails.log.verbose('(You are seeing this log message because no callback was provided to `.lift()`.)');
  };
